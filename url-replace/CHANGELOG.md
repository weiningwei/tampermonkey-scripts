...
- 收起 / 展开把手由方形按钮改为**条状小把手**（宽 20px、高 40px，居中显示双箭头 `»` / `«`），大幅减少对页面的遮挡，更贴合「把手」的定位。

## [0.11.6] - 2026-08-30

### Fixed

- 彻底修复收起/展开后把手在任意位置连续操作仍会漂移：根因是工具栏采用 `left/top` 左上锚定，而把手（最右侧按钮）在右缘，收起/展开会改变工具栏宽度，原实现每次都要「重新测量把手位置 → 补偿 left/top」，依赖 `getBoundingClientRect` 的亚像素精度，缩放/高 DPR 下四舍五入逐次累积、且靠边时被钳制放大。现改为 `right/bottom` 右下锚定并持久化 `{right, bottom}`：把手右缘恒等于工具栏右缘，收起/展开仅改变左侧延伸，把手位置完全由 CSS 决定、不再需要任何 JS 测量补偿，从根源消除漂移；同时保留对旧版 `{left, top}` 存储格式的自动换算兼容。
...
- 修复右下角默认（右锚定）位置连续收起/展开时把手仍会漂移：把手按钮文案 `«` 与 `»` 字形宽度不一致，导致按钮宽度变化、右锚定时把手中心发生位移。现改为给把手固定宽度并居中文本，保证收起/展开时按钮尺寸恒定。

## [0.11.3] - 2026-08-30

### Fixed

- 修复展开时把手位置改变：原实现用工具栏右缘定位并在左侧越界时被钳制到屏幕左缘，导致把手（收起/展开按钮）被推移（如拖到左缘附近再展开会明显移位）。现改为按把手中心定位，收起/展开时把手中心始终不变。
