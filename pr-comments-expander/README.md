...
- **不干扰阅读位置**：页面自身在展开讨论后会把视口滚到对应位置，脚本代点的折叠块往往不在当前阅读位置；代点期间编程式滚动 API（`scrollIntoView` / `scrollTo` 等）被临时静音，展开过程视口保持不动，任务结束后恢复。滚轮 / 触摸 / 键盘滚动不受影响。
...
1. 安装脚本后打开任意 GitCode / AtomGit 的 PR 页面，脚本会自动展开所有折叠评论，右下角实时显示进度（`展开中… N`）与结果（`已展开 N 处`）。计数为当前页面累计值：异步晚到或展开后新出现的折叠块继续累加，跳转到其他页面后清零。
